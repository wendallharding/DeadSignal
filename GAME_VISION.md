...
Starting with the dedicated lighting runs after landmark finish, lighting becomes a defining compositional layer rather than uniform illumination behind materials. Use controlled darkness between localized pools of authored task light so machinery, traversal lanes, and room purpose are revealed deliberately. Each major room should have one dominant focal-light role tied to its verb—such as amber machinery, cold coolant processing, restrained furnace heat, red security containment, or magenta siphon infrastructure—while adjacent rooms remain distinguishable by value, direction, and source shape as well as hue. Darkness must frame decisions without hiding the drone, projectiles, telegraphs, objectives, prompts, hazards, or the only escape lane.

Restoring the station should visibly relight local machinery, thresholds, and nearby structure instead of relying primarily on broad cyan territory coverage. Cyan powered territory remains authoritative gameplay language, but it should support local room identity rather than become the largest or brightest element in ordinary frames. Prefer state-driven changes to practical lights, emissive fixtures, spill direction, and shadow hierarchy over adding more full-floor glow. The intended visual journey is a dark industrial station progressively revealed and repainted by working machinery, with every lighting change preserving Reduced Flashes, Steady Camera, color-independent state communication, and the production-camera combat silhouette hierarchy.

A steady player-centered traversal pool and an aim-following directional beam are part of that lighting language: they should keep the drone and immediate decision space readable while fixed practical lights reveal machinery, routes, and room identity through brighter pools, directional spill, and selective soft shadows. They must complement rather than erase the controlled darkness between authored sources.
...
Fast-paced mandatory interactions should not introduce a last-second Signal affordability check unless the transaction is clearly previewed and creates an intentional route decision. Mission prerequisites, spatial commitment, combat, and interaction timing remain preferable gates. Relay Foundry commissioning and Spine installation therefore cost no Signal and grant their authored refills as forward momentum; the separately previewed opening Central transaction and optional costs remain available for later economy evaluation. In-range contextual prompts sit beside the on-objective icon so the action remains readable without pulling attention to the bottom edge during fast combat.
...
The far Spine berth is now a real third Signal tower after the Relay is online. Its installation is free after the required Relay-payload and venting sequence, restores 34, and creates a smaller 6.2-metre powered foothold around the deepest authored landmark. Instead of opening a fourth choice prompt, activation evolves the Relay calibration already chosen: Piercing Pulse gains a third different aligned target, while Controlled Ricochet gains a second legal cover bank. The outward traversal, authored prerequisites, and combat pressure carry the commitment; the final mandatory interaction no longer adds a real-time affordability check.
...
The planned evolution of Room B is a bounded 75–90-second arcade-pressure encounter inspired by the enemy ecology, formation grammar, population cadence, readable chaos, and arena-to-player scale of *Geometry Wars: Retro Evolved*. It will use original DEAD SIGNAL enemy names, station-derived meshes, effects, audio, tuning, economy, and presentation rather than copying the reference game's expressive assets. A Retro Baseline profile will isolate analogous movement and spawning behavior before a separate DEAD SIGNAL Remix introduces established specialists and Signal consequences. The current three-phase combat is untuned scaffolding and will not be preserved as a gameplay profile; only its Room A/B/C, door, navigation, reward, reset, and run-completion integration contracts remain requirements. Room B must be widened horizontally to a measured reference-relative target before spawn and population tuning; its vertical extent remains initially unchanged unless the same measurement or human circulation evidence shows a deficiency. Camera framing may pull back and be recalibrated for the wider arena, but this system must never alter camera yaw. The staged program in `SECURITY_TRIAL_ARCADE_COMBAT_DESIGN.md` must pass deterministic rules, viewport/spawn safety, performance, economy, accessibility, complete-route, build, and matched human-play gates. The full pattern remains exclusive to Room B unless later evidence supports a new explicit product decision.
