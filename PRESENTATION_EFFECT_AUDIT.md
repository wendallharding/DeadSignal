...
| Enemy / shield / player / wall impact | `CombatFeedbackController` creates one sprite object and one spark particle object per call; decisive hits add a second sprite | sprite 0.22 s; sparks 0.08–0.18 s; camera impulse 0.16 s; hit-stop 0.035/0.06 s | Highest-priority pool target. Prewarm 12 sprite slots and 12 spark slots; hard cap 16 of each; reuse oldest non-decisive visual rather than allocate above cap | Enemy hit white/cyan, shield cyan/white, player damage red, Sapper magenta, wall amber/white. Reduced Flashes caps sprite alpha at 0.3 and uses 3 rather than 6 sparks. Steady Camera must immediately cancel and restore camera offset |
| Enemy purge reaction | `CombatFeedbackController` combines a decisive impact, target scale punch, and Signal-recovery burst | impact 0.22 s; scale punch 0.16 s; recovery burst 0.22 s | Share the impact pool. Target reactions are data-only list entries; cap at active threat population. Recovery burst may reserve two pool slots so it cannot disappear behind ordinary hits | Purge must be larger/longer in silhouette, not whiter than the 0.3 Reduced-Flashes cap; no additional hit-stop or full-screen flash |
| Chain-arc links | `CombatFeedbackController` creates one `LineRenderer` object per link | 0.18 s | Prewarm 6, hard cap 8, reuse oldest inactive link; material remains shared | Cyan with transparent end; Reduced Flashes caps alpha at 0.3; links must remain thinner than enemy telegraphs and may not cross HUD prompts as screen-space overlays |
