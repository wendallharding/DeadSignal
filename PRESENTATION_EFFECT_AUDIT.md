...
| Weapon transformation / evolution | `WeaponTransformationFeedbackController` selects one authored Piercing or Ricochet glyph after the existing choice resolves | 0.9 s; one slot for transformation, two concentric slots for Spine evolution | Two slots are created once with the runtime owner and reused; the presenter cannot select or evolve a weapon | Piercing uses a straight three-ring lance; Ricochet uses an angular two-bank path. Standard alpha 0.66, Reduced Flashes 0.28; world-local, transparent, and camera-static |
| Extraction and terminal outcomes | `ExtractionOutcomeFeedbackController` reuses the station-state glyph for one fixed Dock progress ring and two terminal event layers | Progress persists only while the uplink is active; completion, victory, and defeat last 0.92 s | One progress owner and two event owners are warmed once; saturation reuses event layers and cannot affect the countdown or outcome | Stable progress is cyan, Overdrive amber, completion amber-to-cyan, victory cyan-to-white, defeat red-to-magenta. Reduced Flashes removes progress pulsing and caps persistent/event alpha at 0.14/0.26; no screen overlay or camera motion |
...
## Implementation result — Run 175

Relay weapon calibration now emits one short world-local glyph at the drone after the existing choice resolves. Piercing Pulse uses a straight cyan-white lance crossing three aligned rings; Controlled Ricochet uses a visibly different angular path through two amber bank points. Spine evolution replays the selected build as two concentric, staggered glyphs, so evolution reads as an amplification of the chosen weapon rather than another generic tower pulse. `WeaponTransformationFeedbackController` prewarms exactly two sprite owners, clears them on pause/disable/restart, writes no camera transform, and observes only successful `SignalOverclockChoice` transitions.

`WeaponTransformationFeedbackTuning` holds the `0.9s` lifetime, `0.66` standard alpha, `0.28` Reduced-Flashes ceiling, and transformation/evolution diameters. Focused Unity evidence proved separate imported textures for both builds, one-slot selection, two-slot evolution, accessibility bounds, pause cleanup, complete runtime composition, and the optional full-extraction journey through Relay selection and Spine evolution. Targeting, target count, ricochet banks, cadence, damage, Signal, enemies, counters, progression, and camera behavior are unchanged. Human comparison at 1280×720, 1600×900, and ultrawide remains required to judge whether both motifs are immediately recognizable without hiding the drone, bolts, telegraphs, prompts, or an escape lane.

## Implementation result — Run 176

The live uplink now owns one fixed world-local ring at the Dock. It advances continuously with the existing countdown, distinguishes Stable cyan from Overdrive amber, and remains a restrained `0.22` maximum-alpha state rather than a screen-space effect. Extraction completion resolves amber to cyan; victory adds a wider cyan-to-white release; defeat contracts red into magenta at the drone. The two short terminal layers last `0.92s`, reuse two warmed owners, and cap at `0.58` standard alpha. Reduced Flashes removes the progress pulse and caps persistent/event alpha at `0.14`/`0.26`; Steady Camera remains authoritative because the presenter never writes a camera transform.

`ExtractionOutcomeFeedbackController` observes only successful existing begin/countdown/completion/outcome mutations. It cannot begin or shorten extraction, change Signal, spawn or purge threats, decide victory/defeat, control focus, or alter pause. Focused Unity evidence proved start/progress/completion/victory/defeat wiring, fixed ownership, saturation reuse, accessibility ceilings, pause cleanup, fresh victory/defeat restarts, and complete runtime composition. The required and optional complete journeys both reached victory with their existing mission contracts. Human comparison at 1280×720, 1600×900, and ultrawide remains required to confirm the Dock ring stays subordinate to threats, bolts, prompts, suppression telegraphs, and one escape lane.
