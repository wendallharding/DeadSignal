...
- The objective indicator now uses frame-rate-independent unscaled-time damping when moving between its edge-card position and the projected on-screen objective. Its first initialized position still snaps, avoiding an arbitrary HUD entrance animation, and the edge connector follows the interpolated card rather than jumping ahead.
- Added designer tuning through `EdgeIndicatorTuning.ObjectiveTransitionSpeed` with the existing asset set to `12`, plus PlayMode coverage that first settles the edge state, verifies the first on-screen frame has not popped directly to the target, and verifies eventual convergence.
