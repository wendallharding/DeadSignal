...
- [x] **Presentation Run P22 — Withdrawal and Dock lighting:** strengthen powered footholds, pursuit landmarks, Departure surge, and extraction uplink progression so the return visibly differs from the outbound route and the Dock remains readable under live threats
- [x] **Presentation Run P23 — Shadow, probe, and post-process consistency:** audit shadow softness/resolution, light leakage, reflection/probe coverage, ambient contribution, bloom, exposure, vignette, and color grading across the representative rooms; fix only evidence-backed discontinuities and preserve accessibility contrast
