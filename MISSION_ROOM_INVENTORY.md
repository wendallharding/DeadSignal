...
| **East Transfer Vault** — top-level `Optional East Salvage Vault`; `(16.7, 0.0)` | Central Maintenance Concourse ↔ Relay Foundry | Current Phase 2 assembly objective. `RunModel.TryAssembleCentralPayload` requires routed feeds; `AuthoredTransferVaultObjective` owns the serialized assembly anchor plus amber available and persistent cyan payload markers. The legacy cache socket remains removed. | Dead territory; Relay Fork routing exposes the amber transfer-cycle marker, then assembly leaves the released Central payload marker cyan and unlocks the temporary compatibility route to Relay. | Seven existing boundary/gate/splitter blockers retain collision and the evolved open east exit; objective markers have no colliders or NavMesh mutation. It remains the only east-chain bridge and the same corridor is used on withdrawal. |
