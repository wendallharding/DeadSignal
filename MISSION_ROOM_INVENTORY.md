...
7. Collect exactly one Spine payload from the temporary hard-coded north/south sockets within Capacitor Spine.
8. Return through Relay and Central powered footholds to the Dock; optionally collect the Furnace-side cache to open the Quench return, cross the Departure surge lane, then complete the live extraction uplink.

The current model requires three towers and three regional payload completions; Central's regional completion includes its two component jobs, Relay Fork routing, transfer-vault assembly, and Central installation, while Relay completion includes Gantry stabilization and Foundry installation. Spine activation now also requires Discharge Trench venting. It still does **not** require Induction, Flux, Convergence, Breaker, Furnace processing, Quench processing, or Rooms A–C. These are the remaining mission-flow gaps that later slices must close without breaking the playable route.
