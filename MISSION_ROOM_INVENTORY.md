...
| Space and authored evidence | Actual adjacency | Current gameplay authority and ownership | Power state | Doors, collision, NavMesh, and return value |
| --- | --- | --- | --- | --- |
| **Relay Foundry** — top-level `Relay Foundry Region`; root `(27.5, 0.0)`, Relay Tower child/anchor on its east side | East Transfer Vault ↔ Capacitor Spine; south opening ↔ Cooling Gantry | Current Phase 3 commissioning objective. `RunModel.TryActivateRelayTower` and world/game orchestration own the one-shot tower state; activation advances to the temporary Relay-payload compatibility step without awarding the later weapon calibration. The authored Relay socket remains available until Cooling Gantry processing and Foundry installation replace that adapter. | Dead until Relay activation; then the 7.2 m tower radius, authored territory, cyan tower core, and signal lines power persistently. | Authored region blockers remain unchanged. Activation disables the collision/projectile/NavMesh-authoritative Relay Return Bulkhead, rebuilds NavMesh, and creates the second powered foothold reused on withdrawal. |
...
4. Activate Relay Tower and choose the weapon calibration.
5. Collect exactly one Relay payload: Relay Foundry or Cooling Gantry; the other retires.
...
The current model requires three towers and three regional payload completions; Central's regional completion now includes its two component jobs, Relay Fork routing, transfer-vault assembly, and Central installation. It still does **not** require Discharge Trench, Induction, Flux, Convergence, Breaker, Furnace processing, Quench processing, or Rooms A–C. These are the remaining mission-flow gaps that later slices must close without breaking the playable route.
