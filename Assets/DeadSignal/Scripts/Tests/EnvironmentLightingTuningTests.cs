...
namespace DeadSignal.Tests
{
    public sealed class EnvironmentLightingTuningTests
    {
        ...
        public void AuthoredTuning_DefinesReadableRolesAccessibilityAndPerformanceCeilings()
        {
            ...
            Assert.That(tuning.LandmarkIntensityMultiplier, Is.GreaterThanOrEqualTo(2f));
            Assert.That(tuning.PlayerTraversalLightIntensity, Is.GreaterThan(tuning.KeyLightIntensity * 4f));
            Assert.That(tuning.PlayerTraversalLightRange, Is.GreaterThanOrEqualTo(12f));
            Assert.That(tuning.PlayerTraversalLightSpotAngle, Is.InRange(70f, 100f));
            Assert.That(tuning.PlayerTraversalLightHeight, Is.GreaterThan(4f));
            Assert.That(tuning.PlayerTraversalLightForwardReach, Is.GreaterThan(3f));
            Assert.That(tuning.PlayerTraversalLightShadowStrength, Is.GreaterThanOrEqualTo(0.65f));
            Assert.That(tuning.PlayerFillLightIntensity, Is.GreaterThan(3f));
            Assert.That(tuning.PlayerFillLightRange, Is.GreaterThanOrEqualTo(7f));
            Assert.That(tuning.MaximumVisibleRealtimeLights, Is.EqualTo(7));
            Assert.That(tuning.MaximumShadowedRealtimeLights, Is.EqualTo(3));
