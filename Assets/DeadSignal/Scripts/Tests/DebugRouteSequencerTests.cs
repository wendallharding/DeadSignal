...
namespace DeadSignal.Tests
{
    public sealed class DebugRouteSequencerTests
    {
        ...
        public void FullExtractionRoute_CoversThreeTowersWeaponEvolutionAndOptionalCache()
        {
            ...
            _completeCurrentStep(sequencer);
            Assert.That(sequencer.CurrentStep.Action, Is.EqualTo(DebugRouteAction.CommitSecurityTrial));
            _completeCurrentStep(sequencer);
            Assert.That(sequencer.CurrentStep.Action, Is.EqualTo(DebugRouteAction.CompleteSecurityTrial));
            _completeCurrentStep(sequencer);
            ...
            _completeCurrentStep(sequencer);
            Assert.That(sequencer.CurrentStep.Location, Is.EqualTo(DebugLocation.CurrentObjective));
            Assert.That(sequencer.CurrentStep.Name, Does.Contain("Relay powered shortcut"));
            _completeCurrentStep(sequencer);
            Assert.That(sequencer.CurrentStep.Location, Is.EqualTo(DebugLocation.CurrentObjective));
            Assert.That(sequencer.CurrentStep.Name, Does.Contain("Transfer-vault return feed"));
            _completeCurrentStep(sequencer);
            Assert.That(sequencer.CurrentStep.Location, Is.EqualTo(DebugLocation.CurrentObjective));
            Assert.That(sequencer.CurrentStep.Name, Does.Contain("Central powered foothold"));
            _completeCurrentStep(sequencer);
            Assert.That(sequencer.CurrentStep.Location, Is.EqualTo(DebugLocation.CurrentObjective));
            Assert.That(sequencer.CurrentStep.Name, Does.Contain("Warden Bay pursuit"));
            _completeCurrentStep(sequencer);
            Assert.That(sequencer.CurrentStep.Location, Is.EqualTo(DebugLocation.CurrentObjective));
